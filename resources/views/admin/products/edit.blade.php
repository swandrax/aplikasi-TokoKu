...
@section('header_title', 'Edit Produk Barang')
...
<div class="max-w-3xl bg-white border border-slate-100 rounded-3xl shadow-sm p-6 space-y-6">
    <div class="border-b border-slate-50 pb-4">
        <h3 class="font-extrabold text-slate-800 text-xs uppercase tracking-wider">Form Edit Produk</h3>
        <p class="text-[10px] text-slate-400 mt-1">Perbarui detail produk dagangan Anda melalui formulir di bawah ini.</p>
    </div>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" placeholder="Contoh: Asus ROG Strix Gaming" required 
            ...
            <div>
                ...
                <input type="number" name="price_sell" id="price_sell" value="{{ old('price_sell', (int) $product->price_sell) }}" placeholder="Contoh: 150000" min="0" required 
                       class="w-full px-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:bg-white text-slate-700 transition-all">
                @error('price_sell')
                    <span class="text-[10px] text-rose-500 font-bold block mt-1.5">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="weight" id="weight" value="{{ old('weight', (int) $product->weight) }}" placeholder="Contoh: 450" min="0" required 
                       class="w-full px-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:bg-white text-slate-700 transition-all">
                @error('weight')
                    <span class="text-[10px] text-rose-500 font-bold block mt-1.5">{{ $message }}</span>
                @enderror
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="5" placeholder="Tuliskan spesifikasi, keunggulan, dan rincian produk..." 
                      class="w-full px-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:bg-white text-slate-700 transition-all">{{ old('description', $product->description) }}</textarea>
            @error('description')
                <span class="text-[10px] text-rose-500 font-bold block mt-1.5">{{ $message }}</span>
            @enderror
        </div>

        <!-- Current image preview & File Upload -->
        <div>
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Foto Produk (Maks 2MB)</label>
            @if($product->image)
                <div class="mb-3">
                    <span class="text-[10px] text-slate-400 block mb-1">Foto Saat Ini:</span>
                    <img src="{{ asset('storage/' . $product->image) }}" alt="Preview" class="w-24 h-24 object-cover rounded-xl border border-slate-100 shadow-sm">
                </div>
            @endif
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" id="is_active" {{ $product->is_active ? 'checked' : '' }} class="w-4 h-4 text-indigo-650 bg-slate-50 border border-slate-200 rounded focus:ring-indigo-500 focus:outline-none">
            <label for="is_active" class="text-xs font-bold text-slate-600 cursor-pointer">Aktifkan Produk</label>
        </div>
