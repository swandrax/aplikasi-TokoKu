...
@section('header_title', 'Registrasi Pengguna / Staff Baru')
...
<div class="max-w-2xl bg-white border border-slate-100 rounded-3xl shadow-sm p-6 space-y-6">
    <div class="border-b border-slate-50 pb-4">
        <h3 class="font-extrabold text-slate-800 text-xs uppercase tracking-wider">Form Registrasi Pengguna</h3>
        <p class="text-[10px] text-slate-400 mt-1">Daftarkan akun admin baru, petugas kasir, atau pembeli umum di sistem TokoKu.</p>
    </div>
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Contoh: Budi Gunawan" required 
        ...
        <!-- Password Field -->
        <div>
            <label for="password" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Kata Sandi Awal</label>
        ...
        <div>
            ...
            <select name="role" id="role" required 
                ...
                <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Super Administrator</option>
                <option value="kasir" {{ old('role') === 'kasir' ? 'selected' : '' }}>Petugas Kasir</option>
                <option value="pembeli" {{ old('role') === 'pembeli' ? 'selected' : '' }}>Pembeli Ritel (Customer)</option>
            </select>
            @error('role')
                <span class="text-[10px] text-rose-500 font-bold block mt-1.5">{{ $message }}</span>
            @enderror
        ...
        <div class="flex items-center gap-2">
            ...
            <label for="is_active" class="text-xs font-bold text-slate-600 cursor-pointer">Aktifkan Akun Langsung</label>
        </div>
